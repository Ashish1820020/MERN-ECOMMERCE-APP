...
const addAddress = catchAsyncError(async (req, res) => {
    ...
    try {
        const {name, phoneNo, pinCode, locality, address, district, state, landmark, altNo, id} = req.body;

        let query = {name, phoneNo, pinCode, locality, address, district, state, landmark, altNo};
...
const updateProfile = catchAsyncError(async (req, res) => {
    ...
    try {
        ...
        if(!match) return res.status(401).json({success: false, msg: "Wrong Password"});
...
module.exports = { logIn, signUp, testController, userAuth, adminAuth, forgotPassword, resetPassword, updateProfile, getAllUsers, 
                deleteUser, addAddress, logout };
