...
const getCartAndWishlistData = catchAsyncError(async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Got cart and Wishlist", ...queryObj });
  } catch (error) {
    // console.log(error);
    return next(new ErrorHandler("Internal Error Occurred", 500));
  }
  ...
  try {
    ...
    if (cartProducts.length > 0) {
      updatedCartData = cartProducts.map((currentElement) => {
        cartTotal +=
          getDiscountedPrice(currentElement.price, currentElement.discount) *
    ...
    if (currentUserCart) {
      await CartData.findByIdAndUpdate(currentUserCart._id, {
        products: updatedCartData,
        cartTotal,
      });
    } else {
      const updatedData = new CartData({
        user: userId,
        products: updatedCartData,
        cartTotal,
      });
    ...
    if (currentUserWishlist) {
      await WishlistData.findByIdAndUpdate(currentUserWishlist._id, {
        wishlist: updatedWishlistData,
      });
    } else {
      const updatedData = new WishlistData({
        user: userId,
        wishlist: updatedWishlistData,
      });
      await updatedData.save();
    ...
    res
      ...
      .json({
        success: true,
        msg: "Products updated to the Cart and wishlist Successfully",
      });
  } catch (error) {
    // console.log(error);
    return next(new ErrorHandler("Internal Error Occurred", 500));
