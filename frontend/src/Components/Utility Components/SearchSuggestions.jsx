...
const SearchSuggestions = ({ products, searchText, isResultLoading, setSuggestionBar }) => {
          ...
          products.map((product)=>{
            return(
              <NavLink to={`/singleproduct/${product._id}`} key={product._id} > 
                <div className="search-result-card" onClick={() => setSuggestionBar(false)}>
                  <img src={product.images[0]} alt="" />
                  <p>{product.name}</p>
                  <p className='price'><FormatPrice price={product.discountedPrice} /></p>
                </div>
              </NavLink>
            )
          })
...
const Suggestions = styled.div`
  ...
  .initial-text{
    ...
    padding: 1rem 0rem;
    box-shadow: 0 0 4px 0px rgba(0,0,0,.6);
  }
